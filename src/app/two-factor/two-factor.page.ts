...
export class TwoFactorPage implements OnInit {
  ...
  isLogin: any;
  two_factor: any;
  constructor(private formBuilder: FormBuilder, private location: Location, private helper: HelperService, private device: Device, private router: Router, private route: ActivatedRoute) {
  ...
  skipData(){
    this.helper.getByKeynew('storetoken', res=>{
      let body: any = {
        ...
        this.helper.postMethod('skip-two-factor', body, async res => {
          console.log(res)
...
console.log(this.otpForm.controls['checkData'].value);
  this.helper.getByKeynew('storetoken', res=>{
    ...
    let two_factor: any = this.isLogin == "true" ?  1 : (! parseInt(this.two_factor));
